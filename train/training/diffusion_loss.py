...
try:
    ...
    class DiscreteDenoiser:  # type: ignore[no-redef]
        ...
        def __init__(
            ...
            linear_start: float = 5e-6,
            linear_end: float = 0.012,
            log_snr_shift: float = 2.4,
        ) -> None:
            if num_idx != 1000:
                raise ValueError(
                    "The local DiscreteDenoiser fallback currently supports num_idx=1000 only."
                )
            ...
            sigmas = sigmas * torch.exp(torch.tensor(log_snr_shift, dtype=torch.float64, device=device))
            self.num_idx = int(num_idx)
...
ScheduleName = Literal["seva_ddpm", "cosine_vp", "rf_linear"]
...
def schedule_alpha_sigma(
    ...
    *,
    ...
    Supported schedules:
      - seva_ddpm: handled in compute_seva_diffusion_loss via DiscreteDenoiser
      - cosine_vp: common VP-style cosine interpolation
      - rf_linear: simple linear interpolation used in rectified-flow style paths
    """
    if schedule == "seva_ddpm":
        raise ValueError(
            "schedule_alpha_sigma() is only for continuous debug schedules. "
            "Use sample_seva_noise_levels() for schedule='seva_ddpm'."
        )
...
def compute_seva_diffusion_loss(
    *,
    ...
    schedule: ScheduleName = "seva_ddpm",
    objective: ObjectiveName = "epsilon",
    cosine_s: float = 0.008,
    ...
    if schedule == "seva_ddpm":
        if objective != "epsilon":
            raise ValueError(
                "schedule='seva_ddpm' trains the raw SEVA wrapper as epsilon prediction. "
                f"Use objective='epsilon', got {objective!r}."
            )

        timesteps, sigma, _denoiser = sample_seva_noise_levels(
            (B, T),
            device=device_obj,
            num_idx=1000,
        ...
        # Match the released SEVA denoiser convention:
        #   x_sigma = x0 + sigma * eps
        #   network input = x_sigma * c_in, where c_in = 1 / sqrt(sigma^2 + 1)
        noisy_latents = latents + sigma_e * noise
        ...
        # SGMWrapper does not consume "replace"; the released DiscreteDenoiser
        # applies it before calling the network, so do the same here.
        replace = moved_c.pop("replace", None)
        if replace is not None:
            replace_x, replace_mask = replace.split((x.shape[1], 1), dim=1)
            replace_x = replace_x.to(device=device_obj, dtype=x.dtype)
            replace_mask = replace_mask.to(device=device_obj, dtype=x.dtype)
            x = x * (1.0 - replace_mask) + replace_x * replace_mask

        sigma_flat = sigma.flatten(0, 1).to(device=device_obj, dtype=x.dtype)
